...
def vocab_counter():
    """The purpose of vocab_counter is to analyze every word in the text (film review) and display the frequency with which it occurs in the text"""
    ...
    # shows the frequency of each word in the text
    print(count.vocabulary_)

def term_frequency():
    """Using TF-IDF shows how important a word is to the text as a whole
    I used this to appropriately weigh words. The output is an array with all of the proportionate
    weights of each word. """
    np.set_printoptions(precision=2)
    tfidf = TfidfTransformer(use_idf=True, norm= 'l2', smooth_idf=True)
    # word relevancy
    # This is done using Term Frequency-Inverse Document Frequency
